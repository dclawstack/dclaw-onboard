...
# Check events
kubectl get events -n dclaw-onboard --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-onboard deployment/dclaw-onboard-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-onboard-db-credentials -n dclaw-onboard
   ```
