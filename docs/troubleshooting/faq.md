...
### How do I update DClaw Onboard?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp onboard --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Onboard without Kubernetes?

Yes, for local development:

```bash
cd dclaw-onboard/frontend && npm run dev
cd dclaw-onboard/backend && uvicorn dclaw_onboard.main:app --reload
```
